...
module Data.HarmonicAnalysis.Internal
  ( -- * Core analysis surface used by the public API
    harmonicAnalysis,
...
import Data.HarmonicAnalysis.Tension (makeDiatonicTensionTable, makeMajorMinorDiatonicTensionTable, makeMajorMinorTensionTable)
...
-- | Select the canonical configuration for a Java configuration number.
baseConfigFor :: Int -> HarmonicAnalysisConfig
baseConfigFor 0 = majorMinorTSDConfig
baseConfigFor 1 = majorMinorDiatonicConfig
baseConfigFor 2 = modalTSDConfig
baseConfigFor 3 = modalDiatonicConfig
baseConfigFor _ = majorMinorTSDConfig

-- | Ensure a supplied configuration matches the expected dimensions for a Java configuration.
normalizeConfigFor :: Int -> HarmonicAnalysisConfig -> HarmonicAnalysisConfig
normalizeConfigFor configNum config =
  let expectedRows = numModesForConfig configNum * numFunctionsForConfig configNum
      expectedLen = expectedRows * numTonalities
   in if V.length (weightTable config) == expectedLen
        then config
        else baseConfigFor configNum
...
harmonicAnalysisWithConfig configNum config pitchSets =
  let normalizedConfig = normalizeConfigFor configNum config
      -- Find chains of thirds for each pitch set
      chains = map findChainsOfThirds pitchSets

      -- Compute Riemann matrices with configuration-specific dimensions
      matrices =
        zipWith
          (\chainList pitchSet -> compute3rdChainForConfig configNum chainList (weightTable normalizedConfig) pitchSet)
          chains
          pitchSets
      ...
      path = viterbiPathForConfig configNum matrices (tensionTable normalizedConfig)
...
makeMajorMinorDiatonicConfig =
  makeRuntimeConfig
    ...
    makeMajorMinorDiatonicTensionTable -- tension table
...
compute3rdChainForConfig configNum chains weights pitchSet =
  let numRows = numModesForConfig configNum * numFunctionsForConfig configNum
      ...
      contributionCount = length chains
      combined = Map.fromListWith (+) updates
      averaged
        | contributionCount <= 0 = Map.empty
        | otherwise = Map.map (/ fromIntegral contributionCount) combined
      rawUpdates = [ (cell, Just value) | (cell, value) <- Map.toList averaged]
      debugUpdates
        | configNum == 0 && pitchSet == Set.fromList (map fromIntegral [2, 6, 11]) =
            trace ("First chord matrix top entries: " ++ show (take 5 (reverse (sortOn snd (Map.toList averaged))))) rawUpdates
        | otherwise = rawUpdates
   in updateMatrix rm debugUpdates
